"""
채널 신규 발굴 (Rising Creators) — YouTube Data API
search.list 없이 카테고리×리전(KR/JP/US) 인기영상만 활용 (일일 ~110 units)
"""
...
def _discover() -> list[dict]:
    ...
    # Step 1: 카테고리별 × 리전별 인기 동영상 (~13 × 3 = 39 units)
    logger.info("Step 1/4: 인기 동영상 수집 (%d 카테고리 × %d 리전)", len(CATEGORY_IDS), len(REGION_CODES))
        ...
        for cat_id in CATEGORY_IDS:
            try:
                ...
                if "QUOTA_EXCEEDED" in str(e):
                    raise
                logger.warning("Region %s Category %s fetch failed: %s", region, cat_id, e)

    # Step 2: 채널 배치 조회 (~8 units)
    logger.info("Step 2/4: 채널 통계 조회")
    channel_ids: set[str] = set()
    for v in all_videos:
        cid = v.get("snippet", {}).get("channelId")
        if cid:
            channel_ids.add(cid)
    logger.info("  고유 채널: %d개", len(channel_ids))
    ...
    for i in range(0, len(channel_list), 50):
        ...
        try:
            ...
            if "QUOTA_EXCEEDED" in str(e):
                raise
            logger.warning("Channel batch fetch failed: %s", e)

    # Step 3: 필터링 + 상위 10개
    logger.info("Step 3/4: 필터링")
    qualified: list[dict] = []
    ...
    logger.info("  적격 채널: %d개 (최대 %d)", len(qualified), MAX_CHANNELS_PER_SCAN)

    if not qualified:
        return []

    # Step 4: 대표 영상 조회 (~30 units)
    logger.info("Step 4/4: 대표 영상 조회 (%d 채널)", len(qualified))
    results: list[dict] = []
    for ch in qualified:
        try:
            ...
            if "QUOTA_EXCEEDED" in str(e):
                raise
            logger.warning("Channel %s video fetch failed: %s", ch["id"], e)
...
def _merge_accumulated(new_channels: list[dict]) -> list[dict]:
    ...
    logger.info("누적 채널: %d개 (신규 %d개 추가)", len(_accumulated_channels), added)
    return _accumulated_channels


def fetch_rising_channels(force: bool = False, api_key: str = "") -> dict:
